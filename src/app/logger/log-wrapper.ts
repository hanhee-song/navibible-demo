...
import { isEqual, cloneDeep, some } from 'lodash';
...
export abstract class LogWrapper implements Component {
  ...
  private static _cloneClean(obj: any): object {
    ...
    for (const key in obj) {
      if (obj.hasOwnProperty(key)) {
        ...
        if (this.isValidKey(key, element)) {
  ...
  private static isValidKey(key, element): boolean {
    const __ = '__';
    if (element === undefined || element === null) return false;
    if (some(INVALID_ENDSWITH, str => element.constructor.name.endsWith(str))) return false;
    if (INVALID_KEYS.has(element.constructor.name)) return false;
    if (key.startsWith(__)) return false;
    if (LogWrapper._hasFunction(element)) return false;
    return true;
  }
